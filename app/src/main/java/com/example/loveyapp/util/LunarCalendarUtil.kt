...
object LunarCalendarUtil {
    ...
    private val lunarDays = listOf("初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
    ...
    private val lunarMonthNumNames = listOf("一", "二", "三", "四", "五", "六",
        "七", "八", "九", "十", "十一", "十二")

    private val tianGan = listOf("甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸")
    private val diZhi = listOf("子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥")
    private val shengXiao = listOf("鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪")

    private val solarMonthDays = intArrayOf(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)
    ...
    fun solarToLunar(solarDate: LocalDate): String {
        val year = solarDate.year
        val month = solarDate.monthValue
        val day = solarDate.dayOfMonth
        val lunar = solarToLunar(year, month, day)
        val ganZhi = getYearGanZhi(lunar[0])
        val shengXiao = getYearShengXiao(lunar[0])
        val monthName = lunarMonthNumNames[lunar[1] - 1]
        return "${ganZhi}${shengXiao}年${monthName}月${lunarDays[lunar[2] - 1]}"
    }

    fun solarToLunarWithYear(solarDate: LocalDate): String {
        val year = solarDate.year
        val month = solarDate.monthValue
        val day = solarDate.dayOfMonth
        val lunar = solarToLunar(year, month, day)
        val monthName = lunarMonthNumNames[lunar[1] - 1]
        return "${lunar[0]}年${monthName}月${lunarDays[lunar[2] - 1]}"
    }

    fun getYearGanZhi(year: Int): String {
        val ganIndex = (year - 4) % 10
        val zhiIndex = (year - 4) % 12
        return tianGan[ganIndex] + diZhi[zhiIndex]
    }

    fun getYearShengXiao(year: Int): String {
        val index = (year - 4) % 12
    ...
    fun parseLunarDate(lunarDateStr: String): Triple<Int, Int, Int>? {
        return try {
            var yearStr = ""
            var monthStr = ""
            var dayStr = ""

            val ganZhiPattern = Regex("[甲乙丙丁戊己庚辛壬癸][子丑寅卯辰巳午未申酉戌亥][鼠牛虎兔龙蛇马羊猴鸡狗猪]年")
            val cleanedStr = if (ganZhiPattern.containsMatchIn(lunarDateStr)) {
                lunarDateStr.replace(ganZhiPattern, "")
            } else {
                lunarDateStr
            }

            val yearMatch = Regex("(\\d+)年").find(cleanedStr)
            if (yearMatch != null) {
                yearStr = yearMatch.groupValues[1]
            } else {
                return null
            }
            val year = yearStr.toInt()

            val afterYear = cleanedStr.substringAfter("年")
            for (i in lunarMonthNumNames.indices) {
                if (afterYear.startsWith(lunarMonthNumNames[i] + "月")) {
                    monthStr = (i + 1).toString()
                    dayStr = afterYear.substringAfter(lunarMonthNumNames[i] + "月")
                    break
                }
            }
            if (monthStr.isEmpty()) {
            ...
            val dayIndex = lunarDays.indexOfFirst { dayStr.startsWith(it) }
            if (dayIndex >= 0) {
                Triple(year, month, dayIndex + 1)
            } else {
                null
            }
        } catch (e: Exception) {
    ...
    fun solarToLunar(year: Int, month: Int, monthDay: Int): IntArray {
        ...
        val data = (year shl 9) or (month shl 5) or monthDay
        var solar11 = 0
        ...
        var offset = solarToInt(year, month, monthDay) - solarToInt(y, m, d)
    ...
    private fun lunarToSolar(year: Int, month: Int, monthDay: Int, isLeapMonth: Boolean): IntArray {
        var dayOffset: Int
        val leapMonth: Int
        var i: Int
        dayOffset = (LUNAR_INFO[year - MIN_YEAR] and 0x001F) - 1
        if (((LUNAR_INFO[year - MIN_YEAR] and 0x0060) shr 5) == 2)
            dayOffset += 31
        i = 1
        while (i < month) {
        ...
        if (dayOffset > 366 || year % 4 != 0 && dayOffset > 365) {
            val newYear = year + 1
            dayOffset -= if (newYear % 4 == 1) 366 else 365
            return convertToSolar(newYear, dayOffset)
        }
        return convertToSolar(year, dayOffset)
    }

    private fun convertToSolar(year: Int, dayOffset: Int): IntArray {
        val solarInfo = IntArray(3)
        for (i in 1..12) {
            var iPos = solarMonthDays[i - 1]
            if (i > 1) {
                for (j in 0 until i - 1) {
                    iPos += solarMonthDays[j]
                }
            }
            if (year % 4 == 0 && i > 2) {
                iPos += 1
            ...
            if (dayOffset <= iPos) {
                ...
                var prevPos = 0
                for (j in 0 until i - 1) {
                    prevPos += solarMonthDays[j]
                }
                if (year % 4 == 0 && i > 2) {
                    prevPos += 1
                }
                solarInfo[2] = dayOffset - prevPos
                break
            }
        }
        solarInfo[0] = year
    ...
    private fun getBitInt(data: Int, len: Int, shift: Int): Int {
        return ((data shr shift) and ((1 shl len) - 1))
    }

    private fun solarToInt(year: Int, month: Int, day: Int): Long {
        var sum: Long = 0
        for (i in 1900 until year) {
            sum += if (isLeapYear(i)) 366 else 365
        }
        for (i in 1 until month) {
            sum += solarMonthDays[i - 1].toLong()
